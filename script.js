...
document.getElementById('addForm').addEventListener('submit', function(e) {
    ...
    // Save to localStorage (simulating file handling)
    saveStudent(student);
    ...
    showToast('✓ Student result saved successfully!', 'success');
});
...
function saveStudent(student) {
    ...
    const existingIndex = students.findIndex(s => s.prn === student.prn);
    if (existingIndex >= 0) {
        ...
        <div class="result-card">
            ...
            <div class="result-info">
                ...
                <div class="info-item">
                    ...
                    <div class="info-value">${student.percentage}%</div>
                </div>
            </div>
    ...
    if (student) {
        ...
        resultDiv.innerHTML = `
            <div class="result-card" style="background: linear-gradient(135deg, #fee2e2 0%, #fecaca 100%); border-color: #fca5a5;">
                ...
                <p style="color: #991b1b; margin-top: 10px;">Please check the PRN number and try again.</p>
            </div>
        `;
...
function loadClassmateData() {
    // Parse CSV data from sample_se1.csv
...
// ==================== SEARCH CLASSMATE ====================
document.getElementById('classmateSearchForm').addEventListener('submit', function(e) {
    e.preventDefault();
    ...
    const classmate = classmateData.find(s => s.prn === searchPRN);
    
    const resultDiv = document.getElementById('classmateResult');
    
    if (classmate) {
        let grade;
        if (classmate.percentage >= 90) grade = 'A';
        else if (classmate.percentage >= 75) grade = 'B';
        else if (classmate.percentage >= 60) grade = 'C';
        else if (classmate.percentage >= 50) grade = 'D';
        else if (classmate.percentage >= 40) grade = 'E';
        else grade = 'F';
        ...
        resultDiv.innerHTML = `
            <div class="result-card">
                ...
                <div class="result-info">
                    <div class="info-item">
                        <div class="info-label">PRN Number</div>
                        <div class="info-value">${classmate.prn}</div>
                    </div>
                    <div class="info-item">
                        <div class="info-label">Student Name</div>
                        <div class="info-value">${classmate.name}</div>
                    </div>
                    <div class="info-item">
                        <div class="info-label">Percentage</div>
                        <div class="info-value">${classmate.percentage}%</div>
                    </div>
                </div>
        ...
        resultDiv.innerHTML = `
            <div class="result-card" style="background: linear-gradient(135deg, #fee2e2 0%, #fecaca 100%); border-color: #fca5a5;">
                <h3 style="color: #dc2626;"><span class="icon">✗</span> Classmate Not Found</h3>
                <p style="color: #991b1b; margin-top: 10px;">No record found for PRN: <strong>${searchPRN}</strong></p>
                <p style="color: #991b1b; margin-top: 10px;">Please check the PRN number and try again.</p>
            </div>
        `;
...
document.addEventListener('DOMContentLoaded', function() {
    loadClassmateData();
    console.log('Student Result Management System loaded successfully!');
    console.log('✓ OOP Concepts: Inheritance, Encapsulation, Polymorphism');
    console.log('✓ File Handling: Data persistence with localStorage');
    console.log('✓ STL Map: Quick PRN-based search functionality');
});
